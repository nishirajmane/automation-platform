    ...
    <div className="min-h-screen flex bg-[#111111] text-white">
      ...
      <aside className="w-64 bg-black border-r border-white/10 hidden md:flex flex-col h-screen sticky top-0">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                  isActive ? 'text-[#C8F04D] bg-white/5' : 'text-zinc-400 hover:text-white hover:bg-white/5'
                }`}
              >
                <Icon className={`h-4 w-4 ${isActive ? 'text-[#C8F04D]' : ''}`} />
                {link.name}
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-white/10">
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-3 w-full hover:bg-white/5 p-2 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-[#C8F04D]">
